...
public void draw() {
  ...
  for (VParticle p : physics.particles) {
    color fill1 = color (p.getRadius()*10, 100);
    color fill2 = color (p.getRadius()*10, 0, 0);
    fill(fill1);
    if (p.x > attr.getAttractor().x-attr.getRadius()/2 && p.x < attr.getAttractor().x+attr.getRadius()/2 && p.y > attr.getAttractor().y-attr.getRadius()/2 && p.y < attr.getAttractor().y+attr.getRadius()/2) {
      fill (fill2);
    }

    rect(p.x, p.y, p.getRadius() * 2, p.getRadius() * 2);
  }
}
